...
namespace TSP.Utils
{
    public class SecondBenchmark
    {
        ...
        public SecondBenchmark(int trueCost, int timeConstraint, int repeats, string inputFileName,
            AnnealMethod? annealMethod, SwapMethod? swapMethod, CoMethod? coMethod, int[] timeConstraints,
            float[] tempModifiers,
        ...
        private void StartBenchmark()
        {
            ...
            foreach (var constraint in _timeConstraints)
            {
                ...
                for (var i = 0; i < 10; i++)
                {
                    if (_coMethod != null)
                    {
                        var genetic = new GeneticAlgorithm(graph, constraint, 1000, _coRate, _mtRate,
            ...
            //     for (var i = 0; i < 10; i++)
            //     {
            //         var genetic = new GeneticAlgorithm(graph, timeConstraint, popSize, 0.8f, 0.01f, CoMethod.OrderedCo,
            //             true);
            //         genetic.Start();
            //         var geneticResult = genetic.GetResults();
